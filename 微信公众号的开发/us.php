...
<section class="weui-flex dynamic" style="flex-direction: column;position: relative;">
    <h3 class="weui-flex__item dynamic_title" style="padding-left: 30px;color: #000000;text-align: right">茂名助力快报</h3>
    <ul class="weui-flex__item dynamic_contents" style="font-size: 14px;">
        <li><a href="http://mp.weixin.qq.com/s/ChW6fPXobjyoBkr8a8R5nw" style="color: #ff4c15;font-size: 13px;">茂名反超十堰！今天16:25，整个茂名的朋友圈都沸腾了！</a></li>
        <li><a href="http://mp.weixin.qq.com/s/6zUy__5l61jJjKbtEzUATA" style="color: #8c16ce;font-size: 13px;">魅力中国城 | 茂名市长原来是这么萌的</a></li>
    </ul>
    <a href="index.html" class="weui-btn weui-btn_mini weui-btn_default" style="position: absolute;top: 15px;left: 15px;"><<&nbsp;实时分析</a>
    <img src="../web/design/loading.gif" alt="" style="position: absolute;bottom: 35%;left: 45%;width: 10%">
</section>
...
//                     $('#gap_percent').html(minusPercent.toFixed(2) + '%');
                    ...
                    if (mmClear > syClear) {
                        ...
                        $('#gap_percent').css({color: '#085282'});
//                        $('#mmScore').css({color: 'red'});
//                        $('#syScore').css({color: 'red'});
...
<script>
    ...
    function getMsg() {
        $.ajax({
            type: 'GET',
            url: 'http://w.mmcity.cn/public/index.php/msg2',
            jsonp: "jsoncallback",
            data: {AccessKey: 'cpuohkzb3ytrc5jsce4ei1ff2cgp34555'},
            success: function (res) {
...
//                $(".dynamic_contents").html("");
                for (var i = 0; i < msg.data.length; i++) {
                    ...
                    $("<li><nobr><a style='color: #000;' href='"+msg.data[i].url +"'>"+ msg.data[i].title +"</a><nobr/></li>").appendTo($(".dynamic_contents"));
